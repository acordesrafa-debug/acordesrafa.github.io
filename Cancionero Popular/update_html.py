...
csv_path = r"c:\Users\rafae\Downloads\Google_AI_Pruebas\acordes_rafa_web\Cancionero Popular\Lista de canciones.csv"
html_path = r"c:\Users\rafae\Downloads\Google_AI_Pruebas\acordes_rafa_web\acordes.html"
...
with open(csv_path, 'r', encoding='utf-8') as f:
    ...
    for row in reader:
        ...
        full_pdf = next((p for p in pdf_files if pdf_base in p), None)
        if full_pdf:
            display_name = re.sub(r'\(.*?\)', '', titulo).strip()
            if not display_name: display_name = titulo
            populares[display_name] = {"pdf": full_pdf, "tutorial": tutorial, "cover": cover}

populares = dict(sorted(populares.items()))

# Build the block
pop_block = "            populares: {\n"
for name, d in populares.items():
    pop_block += f'                "{name}": {{ pdf: "{d["pdf"]}", tutorial: "{d["tutorial"]}", cover: "{d["cover"]}" }},\n'
pop_block = pop_block.rstrip(",\n") + "\n            },"

with open(html_path, 'r', encoding='utf-8') as f:
    html_content = f.read()

# Replace the populares section
pattern = re.compile(r'populares: \{.*?\},', re.DOTALL)
new_html = pattern.sub(pop_block, html_content)

with open(html_path, 'w', encoding='utf-8') as f:
    f.write(new_html)

print("HTML_UPDATED_SUCCESSFULLY")
